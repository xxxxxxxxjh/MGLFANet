...
class DifferenceEnhancementModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 特征差异计算层
        self.diff_conv = nn.Conv2d(in_channels // 2, out_channels, kernel_size=3, padding=1, bias=False)
        ...
        # 调整特征通道数，以保持与输入一致
        self.adjust_conv = nn.Conv2d(in_channels + out_channels, out_channels, kernel_size=1, bias=True)
        
        self.act = nn.GELU()

    def forward(self, x):
        # 假设输入x的形状为 (B, C, H, W)，其中C = in_channels
        # 将输入分为两个特征图
        C_half = self.in_channels // 2
        f1 = x[:, :C_half, :, :]
        f2 = x[:, C_half:, :, :]
        ...
        # 特征融合
        combined_features = torch.cat([x, enhanced_diff], dim=1)
        
        # 调整通道数
        adjusted_features = self.adjust_conv(combined_features)
        
        # 应用GELU激活函数
        fused_features = self.act(adjusted_features)

...
from models.MGLFANet import MGLFANet
###############################################################################
# Helper Functions
###############################################################################
...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif args.net_G == "MGLFANet":
        net = MGLFANet(dec_embed_dim=args.embed_dim)
    else:
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        dim = 32
        mlp_dim = 2 * dim
        self.coordatt = CoordAtt(inp=32, oup=32)  # 确保匹配 ResNet 输出通道数
        ...
        # 新增：DFFM 模块，用于对特征差分后的融合
        self.dffm = DFFM(in_channels=32, reduction=16)
    ...
    def forward(self, x1, x2):
        ...
        x1 = self.forward_single(x1)
        x2 = self.forward_single(x2)
        x1 = self.coordatt(x1)  # 新增：对 x1 增加 CoordAtt 模块
        x2 = self.coordatt(x2)  # 新增：对 x2 增加 CoordAtt 模块
